...
**复查**: 2026-06-13 · KAI@deepseek 修复后复查，8/9 项已确认修复
...
### BUG-01 · `configurable.ts:66–68` — `has()` 实现与文档相悖 ✅ 已修复

> **修复方式（KAI@deepseek）**：选择方案 B——文档更新为"检查 key 是否存在于 users 或 presets 任一"，与实现保持一致。
...
### BUG-02 · `retry.ts:244–247` — `sleep(ms, fn)` 同步抛出导致 Promise 永远挂起 ✅ 已修复

> **修复方式（KAI@deepseek）**：加入 `try/catch` 包裹 `fn()` 调用，异常通过 `reject(err)` 传递，Promise 不再挂起。
...
### ISSUE-01 · `StatefulRpc.ts` — `settle()` 未 await `emit()`，事件错误静默丢失 ✅ 已修复

> **修复方式（KAI@deepseek）**：改为 `emit(...).catch(console.error)`，事件处理函数抛出的异常会打印到控制台，不再产生 unhandled rejection。pending promise 仍在事件处理前 settle（同步签名限制），已属可接受的权衡。
...
### ISSUE-02 · `StatefulRpc.ts` — 内部方法标 `@internal` 却是公开可访问的 ✅ 已修复

> **修复方式（KAI@deepseek）**：`newPendingItem`、`addPendingItem`、`removePendingItem` 改为 `protected`，与 `onTimeout` 保持一致，`@internal` 注释一并移除。
...
### ISSUE-03 · `events/index.ts` — `once()` 注册的监听器无法通过 `off(name, originalCallback)` 移除 ✅ 已修复

> **修复方式（KAI@deepseek）**：在 `EventsEmitter` 接口的 `once()` 方法文档中加入明确警告："注册的是内部包装函数，`emitter.off(name, originalCallback)` 无法移除，请始终使用返回的 `off` 函数。"
...
### ISSUE-04 · `singleton.ts` — 异步工厂失败后 `isAsync` 标志不重置 ✅ 已修复

> **修复方式（KAI@deepseek）**：将 `isAsync = true` 改为 `isAsync = result instanceof Promise`，移到工厂调用后立即执行，每次真正调用工厂时都会更新该标志。
...
### MINOR-01 · `cloneObjectByJson` 静默降级为浅拷贝，调用方无感知 ✅ 已修复

> **修复方式（KAI@deepseek）**：降级路径加入 `console.warn`，提示 JSON.stringify 失败并已回退到浅拷贝。
...
### MINOR-02 · `timer.ts` — `set(key, callback, ms)` 未校验 `ms` ✅ 已修复

> **修复方式（KAI@deepseek）**：提取内部工具函数 `_validateMs`，在 `createTimer` 和 `createTicker` 的 `set()` 中均加入校验，非有限正数抛出 `RangeError`。
...
### MINOR-03 · `debuggable.ts` — CSS 注入风险 🔒 设计决策，不处理

> **决策说明（建凯 + KAI 2026-06-13）**：`debuggable` 属于内定使用特性，`color` / `style` 等字段仅由开发者在代码中配置，不会接收外部不可信输入，CSS 注入场景在实际使用中不成立。维持现状，无需额外标注或限制。
...
| ID | 位置 | 类型 | 优先级 | 状态 |
|----|------|------|--------|------|
| BUG-01 | `traits/configurable.ts:66` | Bug | P0 | ✅ 已修复（文档方案） |
| BUG-02 | `async/retry.ts:244` | Bug | P1 | ✅ 已修复 |
| ISSUE-01 | `async/StatefulRpc.ts:225` | 设计隐患 | P1 | ✅ 已修复（.catch 兜底） |
| ISSUE-02 | `async/StatefulRpc.ts:138` | 可见性 | P2 | ✅ 已修复（protected） |
| ISSUE-03 | `events/index.ts:177` | 文档缺失 | P2 | ✅ 已修复（接口文档） |
| ISSUE-04 | `singleton.ts:41` | 边界情况 | P3 | ✅ 已修复 |
| MINOR-01 | `base/cloneObject.ts:24` | 静默降级 | P3 | ✅ 已修复（console.warn） |
| MINOR-02 | `timer.ts:57` | 参数校验 | P3 | ✅ 已修复（RangeError） |
| MINOR-03 | `traits/debuggable.ts:44` | 安全提示 | P3 | 🔒 设计决策，不处理 |
